...
public class SubtractionProcessor implements Processor {
    ...
    public boolean applyTo(String message) {
        return message != null && message.trim().length() > 1 && message.trim().substring(1).contains(Operator.SUBTRACTION.getSymbol());
    }

    @Override
    public String process(String message) {
        Operation op = ValidatorOperations.buildOperation(message, Operator.SUBTRACTION);
